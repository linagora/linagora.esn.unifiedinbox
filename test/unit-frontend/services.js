...
describe('The Unified Inbox Angular module services', function() {
  ...
  describe('The inboxSharedMailboxesService service', function() {
    ...
    var $rootScope, inboxSharedMailboxesService, inboxConfigMock, INBOX_HIDDEN_SHAREDMAILBOXES_CONFIG_KEY, configHiddenSharedCalls, configServiceMock;
    ...
    beforeEach(module(function($provide) {
      ...
      configServiceMock = { set: sinon.spy() };
      $provide.value('esnUserConfigurationService', configServiceMock);
    }));
    ...
    describe('The hideNewMailboxes function', function() {
      ...
      it('should save new config through esnUserConfigurationService when non-empty mailbox collection is provided', function(done) {
        inboxSharedMailboxesService.hideNewMailboxes([{ id: '#1', name: 'Shared #1' }, { id: '#2', name: 'Shared #2' }])
          .then(function() {
            expect(configServiceMock.set).to.have.been.calledOnce;
            expect(configServiceMock.set).to.have.been.calledWith([{
      ...
      it('should save new merged mailbox ids when mailboxes includes existing ids', function(done) {
        ...
        inboxSharedMailboxesService.hideNewMailboxes([{ id: '#1', name: 'Shared #1' }, { id: '#2', name: 'Shared #2' }, { id: '#3', name: 'Shared #3' }])
          .then(function() {
            expect(configServiceMock.set).to.have.been.calledOnce;
            expect(configServiceMock.set).to.have.been.calledWith([{
      ...
      it('should merge old hidden mailboxes list with provided list', function(done) {
        inboxConfigMock[INBOX_HIDDEN_SHAREDMAILBOXES_CONFIG_KEY] = { '#2': true };
        inboxSharedMailboxesService.hideNewMailboxes([{ id: '#1', name: 'Shared #1' }, { id: '#3', name: 'Shared #3' }])
          .then(function() {
            expect(configServiceMock.set).to.have.been.calledOnce;
            expect(configServiceMock.set).to.have.been.calledWith([{
              name: INBOX_HIDDEN_SHAREDMAILBOXES_CONFIG_KEY, value: { '#1': true, '#2': true, '#3': true }
    ...
    describe('The setHiddenMailboxes function', function() {
        ...
        it('should save new config through esnUserConfigurationService when non-empty mailbox collection is provided', function(done) {
          inboxSharedMailboxesService.setHiddenMailboxes([{ id: '#1', name: 'Shared #1' }, { id: '#2', name: 'Shared #2' }])
            .then(function() {
              expect(configServiceMock.set).to.have.been.calledOnce;
              expect(configServiceMock.set).to.have.been.calledWith([{
                name: INBOX_HIDDEN_SHAREDMAILBOXES_CONFIG_KEY, value: { '#1': true, '#2': true }
        ...
        it('should only store provided hidden mailboxes list, overriding current list', function(done) {
          inboxConfigMock[INBOX_HIDDEN_SHAREDMAILBOXES_CONFIG_KEY] = { '#2': true };
          inboxSharedMailboxesService.setHiddenMailboxes([{ id: '#1', name: 'Shared #1' }, { id: '#3', name: 'Shared #3' }])
            .then(function() {
              expect(configServiceMock.set).to.have.been.calledOnce;
              expect(configServiceMock.set).to.have.been.calledWith([{
                name: INBOX_HIDDEN_SHAREDMAILBOXES_CONFIG_KEY, value: { '#1': true, '#3': true }
