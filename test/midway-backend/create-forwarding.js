...
describe('The forwarding creation API', function() {
  ...
  describe('PUT /forwardings', function() {
      ...
      helpers.requireBackend('core/esn-config')('forwarding').inModule('linagora.esn.unifiedinbox')
        .store(true, helpers.callbacks.noErrorAnd(() => {
          helpers.api.loginAsUser(app, user.emails[0], password, (err, requestAsMember) => {
            ...
            const req = requestAsMember(request(app).put(API_PATH));
    ...
    it('should return 403 if request to keep local copy while local copy is not enabled', function(done) {
      helpers.requireBackend('core/esn-config')('forwarding').inModule('linagora.esn.unifiedinbox')
        .store(true, helpers.callbacks.noErrorAnd(() => {
          helpers.api.loginAsUser(app, user.emails[0], password, (err, requestAsMember) => {
            expect(err).to.not.exist;
            ...
            req.send({ forwarding: user.emails[0] });
            req.expect(403);
            req.end((err, res) => {
              expect(err).to.not.exist;
              expect(res.body.error.details).to.equal('You are not allowed to add your email as a forwarding');
  ...
  describe('PUT /forwardings/users/:uuid', function() {
    it('should return 401 if not logged in', function(done) {
      request(app)
        .put(API_PATH + '/users/' + user._id + '?domain_id=' + domain._id)
        .expect(401)
        .end(done);
    });
      ...
      helpers.requireBackend('core/esn-config')('forwarding').inModule('linagora.esn.unifiedinbox')
        .store(true, helpers.callbacks.noErrorAnd(() => {
          helpers.api.loginAsUser(app, user1.emails[0], password, (err, requestAsMember) => {
            expect(err).to.not.exist;
            const req = requestAsMember(request(app).put(API_PATH + '/users/' + user._id + '?domain_id=' + domain._id));
    ...
    it('should return 403 if domain admin try to create forwardings for the user that is not domain member', function(done) {
      helpers.requireBackend('core/esn-config')('forwarding').inModule('linagora.esn.unifiedinbox')
        .store(true, helpers.callbacks.noErrorAnd(() => {
          helpers.api.applyDomainDeployment('linagora_test_domain2', function(err, deployedModels2) {
            if (err) {
              return done(err);
            }
            expect(err).to.not.exist;
            ...
            helpers.api.loginAsUser(app, user.emails[0], password, (err, requestAsMember) => {
              expect(err).to.not.exist;
              const req = requestAsMember(request(app).put(API_PATH + '/users/' + userDomain2._id + '?domain_id=' + domain._id));
